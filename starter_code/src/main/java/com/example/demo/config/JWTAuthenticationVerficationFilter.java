...
public class JWTAuthenticationVerficationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest req) {
        String token = req.getHeader(ApplicationConstants.HEADER_STRING);
        if (token != null) {
            String user = JWT.require(HMAC512(ApplicationConstants.SECRET.getBytes())).build()
                    .verify(token.replace(ApplicationConstants.TOKEN_PREFIX, ""))
                    .getSubject();
            if (user != null) {
